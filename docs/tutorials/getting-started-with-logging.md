...
For most users, our bootstrap script provides the fastest path to deployment:
...
3. **Deploy Observability Operator**
   ```bash
...
```bash
# 1. Preview the bootstrap process
./scripts/bootstrap-environment.sh dev --dry-run

# 2. Run the bootstrap (creates AWS resources + ArgoCD apps)  
./scripts/bootstrap-environment.sh dev

# 3. Manual verification phase (prompted by script)
# - Verify AWS S3 bucket and IAM resources
# - Check External Secrets synchronization  
# - Validate ArgoCD application registration
...
```bash
# 1. Always preview first for production
./scripts/bootstrap-environment.sh production --dry-run

# 2. Run the bootstrap with extra scrutiny
./scripts/bootstrap-environment.sh production

# 3. Extended manual verification (required for production)
# - Security review of all created resources
# - Compliance validation 
# - Change approval process

# 4. Controlled GitOps deployment (manual ArgoCD preferred)
# Use ArgoCD UI for production deployments
```
