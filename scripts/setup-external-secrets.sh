...
create_aws_credentials_secret() {
    ...
    # Ensure external-secrets-system namespace exists
    oc create namespace external-secrets-system --dry-run=client -o yaml | oc apply -f -
    
    log "Creating aws-credentials secret in external-secrets-system namespace"
    
    # Delete existing secret if it exists
    oc delete secret aws-credentials -n external-secrets-system 2>/dev/null || true
...
verify_setup() {
    ...
    if oc get secret logging-loki-s3 -n openshift-logging &> /dev/null; then
        ...
        log "Secret keys:"
        oc get secret logging-loki-s3 -n openshift-logging -o jsonpath='{.data}' | jq -r 'keys[]'
    else
...
Components Deployed:
  ...
  ✓ ExternalSecret: logging-loki-s3 (in openshift-logging namespace)
  ✓ AWS credentials secret (in external-secrets-system namespace)
...
Next Steps:
  ...
  3. Loki can use the logging-loki-s3 secret for storage access

Verification Commands:
  oc get clustersecretstore aws-secrets-manager
  oc get externalsecret logging-loki-s3 -n openshift-logging
